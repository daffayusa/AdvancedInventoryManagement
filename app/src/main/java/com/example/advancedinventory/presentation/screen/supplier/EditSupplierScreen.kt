...
fun EditSupplierScreen(
    ...
    // Launch effect untuk memuat detail supplier saat layar dimuat
    LaunchedEffect(supplierId) {
        viewModel.fetchSupplierDetail(FirebaseFirestore.getInstance().collection("suppliers").document(supplierId))
    }

    // Isi state dengan data supplier jika sudah tersedia
    LaunchedEffect(supplier) {
        supplier?.let {
            namaState.value = it.nama
            kontakState.value = it.kontak
            alamatState.value = it.alamat
        }
    }
    ...
    Column(
        ...
        LargeBtn(
            ...
            onClick = {
                ...
                viewModel.updateSupplier(supplierId, namaState.value, kontakState.value, alamatState.value)
